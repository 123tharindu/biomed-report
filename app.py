...
for i in range(st.session_state.num_instruments):
    st.markdown(
        f"<div class='instrument-card'><b>🔪 Instrument Entry #{i+1}</b>",
        unsafe_allow_html=True,
    )
    ...
    inst_item = {}
    with col1:
        inst_item["image"] = st.file_uploader(
            f"Upload Image #{i+1}",
            type=["jpg", "png", "jpeg"],
            key=f"uploader_{i}",
        )
    ...
    with col2:
        ...
        if is_custom:
            ...
            inst_name = st.text_input(
                f"Instrument Description #{i+1}", key=f"name_{i}"
            )
        else:
            art_no = st.selectbox(
        ...
        if inst_item["image"] and GEMINI_API_KEY:
            if st.button(f"✨ AI Auto-Detect Damage #{i+1}", key=f"ai_btn_{i}"):
                with st.spinner("Analyzing with Gemini AI..."):
                    ai_dam, ai_rec = analyze_damage_with_ai(
                        inst_item["image"], inst_item["name"]
                    )
        ...
        inst_item["damage"] = st.text_area(
            f"Damage Details #{i+1}", key=f"dam_{i}", height=80
        )
        ...
        rec_opts = [
            ...
            "Service",
            "Repair",
            "Upgrade / New System Required",
            "OK",
        ]
        inst_item["recommendation"] = st.selectbox(
            f"Recommendation #{i+1}", options=rec_opts, key=f"rec_{i}"
        )
...
# ==========================================
if st.button(
    "📄 Generate PDF Report & Sync Summary",
    type="primary",
    ...
    with st.spinner("Generating PDF Report & Syncing to Google Sheet..."):
        ...
        company_info = [
            Paragraph(
                "BIOMED INTERNATIONAL (PVT) LTD", company_name_style
            ),
            Paragraph(
                "AESCULAP Division | Colombo 03, Sri Lanka", company_sub_style
            ),
        ]
        logo_img = (
            RLImage("bmi_logo.png", width=65, height=32)
            if os.path.exists("bmi_logo.png")
            else Paragraph("<b>BMI</b>", company_name_style)
        )
                ...
                [
                    ...
                    [
                        Paragraph(
                            ...
                            ParagraphStyle(
                                "T", parent=company_name_style, alignment=2
                            ),
                        ),
                        Paragraph(
                            "LAP SCAN DIAGNOSTICS",
                            ParagraphStyle(
                                "S", parent=company_sub_style, alignment=2
                            ),
                        ),
                    ],
                ]
            ],
            colWidths=[70, 285, 200],
        )
        t_header.setStyle(
            TableStyle([
        ...
        disp_engineer = (
            engineer_val.strip()
            if engineer_val.strip()
            else "Biomed Technical Team"
        )
        ...
        meta_data = [
            [
                Paragraph("Customer / Hospital:", label_style),
                Paragraph(disp_hospital, value_style),
                Paragraph("Brand:", label_style),
                Paragraph("Aesculap", value_style),
            ],
            [
                Paragraph("Inspection Date:", label_style),
                Paragraph(date_str, value_style),
                Paragraph("System / Set:", label_style),
                Paragraph("Laparoscopy", value_style),
            ],
            [
                Paragraph("Engineer Name:", label_style),
                Paragraph(disp_engineer, value_style),
                Paragraph("Report No:", label_style),
                Paragraph(disp_rep_no, value_style),
            ],
            [
                Paragraph("Department:", label_style),
                Paragraph(dept_val, value_style),
                Paragraph("Scope S/N:", label_style),
        ...
        for idx, item in enumerate(instruments_data):
            ...
            rec_color = (
                ...
                if item["recommendation"] == "Replace"
                    ...
                    "#D35400"
                    if item["recommendation"] in ["Service", "Repair"]
                    else "#27AE60"
                )
            )
            if item["recommendation"] == "Replace":
            ...
            table_data.append([
                ...
                Paragraph(
                    f"<b><font color='{rec_color}'>{item['recommendation'].upper()}</font></b>",
                    cell_center,
                ),
            ])
        ...
        sig_data = [
            [
                ...
                Paragraph(
                    "<b>Customer Acknowledgment / Hospital Stamp:</b>",
                    sig_title_style,
                ),
            ],
            ...
            [
                Paragraph(
                    f"........................................................<br/><b>Service Engineer:</b> {disp_engineer}<br/>Biomed International (Pvt) Ltd",
                    sig_text_style,
                ),
                Paragraph(
                    "........................................................<br/><b>Authorized Signature & Stamp</b><br/>Hospital / Theatre Unit",
                    sig_text_style,
                ),
            ],
        ]
