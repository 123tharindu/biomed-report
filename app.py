...
# --- PERSISTENT SESSION STATE INITIALIZATION (DATA LOSS PREVENTION) ---
...
with tab1:
    ...
    # META INFORMATION (STORED IN PERSISTENT SESSION STATE)
    ...
    # PDF GENERATION & SAVE TO HISTORY
    if st.button("📄 Generate & Save Official PDF Report", type="primary", use_container_width=True):
        with st.spinner("Generating PDF Report & Saving to History..."):
            buffer = io.BytesIO()
            ...
            th_style = ParagraphStyle('TH', parent=cell_style, fontSize=8, leading=10, textColor=colors.white, fontName='Helvetica-Bold', alignment=1)
            ...
            # Table Data
            table_data = [[Paragraph("#", th_style), Paragraph("PHOTO", th_style), Paragraph("ARTICLE NO", th_style), Paragraph("INSTRUMENT NAME", th_style), Paragraph("DETAILS OF DAMAGE", th_style), Paragraph("RECOMMENDATION", th_style)]]
            ...
            for idx, item in enumerate(st.session_state.form_data["instruments"]):
                ...
                if item["image"]:
                    ...
                    img_cell = RLImage(t_path, width=120, height=120)
                ...
                table_data.append([
                    Paragraph(str(idx+1), cell_center), img_cell, Paragraph(f"<b>{art_no}</b>", cell_style),
                    Paragraph(item["name"], cell_style), Paragraph(item["damage"].replace('\n', '<br/>'), cell_style),
                    Paragraph(f"<b><font color='{rec_color}'>{item['recommendation'].upper()}</font></b>", cell_center)
                ])

            t_main = Table(table_data, colWidths=[20, 125, 65, 110, 130, 85])
            t_main.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), navy_primary), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#BAC7D5')), ('VALIGN', (0,0), (-1,-1), 'MIDDLE')]))
            story.append(t_main)
            ...
            rep_no = st.session_state.form_data['report_no'] if st.session_state.form_data['report_no'] else 'REF-GEN'
            ...
            st.download_button("📥 Download Generated PDF", data=pdf_bytes, file_name=f"Lap_Report_{rep_no}.pdf", mime="application/pdf")
...
with tab2:
    ...
    if len(st.session_state.saved_reports_history) > 0:
        ...
        st.info("තවම කිසිදු Report එකක් Save කර නොමැත. Tab 1 එකෙන් Form එක Fill කර PDF එකක් Generate කරන්න.")
